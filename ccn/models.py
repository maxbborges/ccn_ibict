...
class Postagens(models.Model):
    ...
    def select (self,tipo,valor,juncao):
        ...
        for i,val in enumerate(valor):
            if val:
                if tipo[i]=='cod_issn' or tipo[i]=='pai_cod':
                    if tipo[i]=='pai_cod':
                        val=paises(val)
                        if not val:
                            continue
...
class Universidades(models.Model):
    ...
    def select (self,codigo):
        ...
        b.BAIRRO, l.DES, b.CEP, bc.NUMERO, bc.TIPO , pc.CONTEUDO,mf.DES AS tipotexto,b.base,b.ariel
...
def paises(nomePais):
    ...
    for p in listaDePaises:
        if nomePais.casefold() in p['nome_pais'].casefold():
            return p['sigla']
